...
cef8ed7 HEAD@{2}: commit: testDel save
....
...
1. 合并后没有新的提交：
  ...
  2. 如果不使用 `--hard` 参数，被合并的分支暂存区（stage）会保留在当前分支的暂存区。你需要
  视情况使用。
...
``` bash
# master/1.txt
add one line for init # checkout new branch here
...
多个文件冲突，需要分别处理后再提交。提交后会自动视为合并成功
...
eg: [exlink](https://stackoverflow.com/questions/2221658/whats-the-difference-between-head-and-head-in-git)
